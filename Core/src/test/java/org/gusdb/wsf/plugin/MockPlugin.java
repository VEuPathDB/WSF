...
public class MockPlugin extends AbstractPlugin {
  ...
  public void validateParameters(PluginRequest request) throws WsfUserException {
    ...
    if (rowSize <= 0)
      throw new WsfUserException("The " + PARAM_ROW_SIZE + " param must be a number greater than 0");
    ...
    if (attachmentSize < 0)
      throw new WsfUserException("The " + PARAM_ATTACHMENT_SIZE +
          " param must be a number greater than, or equal to, 0");
  }

  @Override
  protected int execute(PluginRequest request, PluginResponse response) throws WsfException {
    // get params
    Map<String, String> params = request.getParams();
    long rowSize = Long.valueOf(params.get(PARAM_ROW_SIZE));
    int attachmentSize = Integer.valueOf(params.get(PARAM_ATTACHMENT_SIZE));
